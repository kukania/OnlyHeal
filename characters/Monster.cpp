...
#include "skillinstance/Instance/MonsterSkillFear.hpp"
#include "skillinstance/Instance/MonsterSkillAttack.hpp"
int Monster::setFindTarget(Character **t) {
	...
	for (int i = 2; i < 6; i++) {
		...
		if (!t[i]->checkDie() &&
			((temp = t[i]->getStatus()->getAgro()) > max)) {
			target = i;
			max	   = temp;
		}
	}
