...
#include "skillinstance\DebuffSkill.h"
#include <string>

Skill*	DebuffSkillFactory::createSkill() {
	Skill* skill = new DebuffSkill();
	return skill;
}

Skill*	DebuffSkillFactory::createSkill(SkillID ID, string name, time_ms cooltime, time_s time, bool multi, float factor) {
	Skill* skill = new DebuffSkill(ID, name, cooltime, time, multi, factor);
	return skill;
}
